...
4. Never use GitHub Actions, workflows, runners or repository automation as remote machine transport.
5. GitHub remains SCM only.
6. Do not ask the Founder to repeat SSH/bootstrap work merely because a chat lacks the SROF binding.
7. If SROF is not exposed in the current runtime, return exactly:
...
10. Every SROF call must respect host capabilities, allowlists and receipt generation.
...
- DCP = retired.
- GitHub Actions transport = forbidden.
...
> REMOTE ACCESS CANON — Use SCIENTIAM SROF when the current runtime exposes it. Canonical endpoint: https://srof.scientiam.com.ar/mcp. Authentication is native OAuth via https://auth.scientiam.com.ar/realms/scientiam-srof with scope srof:read. DCP is retired. GitHub Actions is not remote transport. If the SROF tool/app is not exposed in this runtime, return TOOLING_GAP and do not invent a fallback. Current SROF MCP tools are read-only and policy/allowlist governed.
